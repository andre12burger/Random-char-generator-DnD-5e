...
# -*- coding: utf-8 -*-
"""
Script final para capturar TODAS as 219 subclasses com tratamento
de casos especiais e versões duplicadas.
"""
...
BOOK_MAPPING = {
    ...
    # Fontes específicas de Blood Hunter
    "Critical Role": "UA",
    "Matt Mercer": "UA",
    "D&D Beyond": "DDB"
}
...
def extract_subclass_table_from_class_page(class_file: Path) -> Dict[str, Set[str]]:
    ...
    try:
        ...
        for table in tables:
            ...
            # Verifica se é uma tabela de subclasses
...
def match_table_entry_to_database_comprehensive(table_subclass_name: str, class_name: str, 
                                               subclasses: Dict[int, Tuple[str, str]]) -> List[int]:
    """Faz match abrangente que pode retornar múltiplas versões da mesma subclasse"""
    ...
    for sub_id, (db_class, db_subclass) in subclasses.items():
        ...
        # 1. Match exato (removendo parênteses e sufixos)
        ...
        if table_core == db_core:
            score = 100
        # 2. Match de núcleo (removendo prefixos de classe)
    ...
    # Retorna todos os matches com score alto, ordenados por score
    matches.sort(key=lambda x: x[1], reverse=True)
    return [match[0] for match in matches]

def add_blood_hunter_manually() -> Dict[int, Set[str]]:
    """Adiciona manualmente as subclasses de Blood Hunter que não estão nas tabelas"""
    # IDs conhecidos do Blood Hunter baseados na análise anterior
    blood_hunter_relations = {
        32: {"UA"},  # Order of the Ghostslayer
        33: {"UA"},  # Order of the Lycan  
        34: {"UA"},  # Order of the Mutant
        35: {"UA"}   # Order of the Profane Soul
    }
    return blood_hunter_relations

def main():
    print("🔍 Capturando TODAS as 219 subclasses com casos especiais...")
    ...
    # Adiciona Blood Hunter manualmente
    ...
    for class_dir in HTML_DIR.iterdir():
        ...
        if not table_data:
            if class_dir.name != "blood-hunter":  # Blood Hunter não tem tabela
                print(f"  ⚠️  Nenhuma tabela de subclasses encontrada")
            continue
        
        # Faz match com o banco de dados (versão abrangente)
        for table_subclass_name, book_codes in table_data.items():
            matching_subclass_ids = match_table_entry_to_database_comprehensive(
                table_subclass_name, class_dir.name, subclasses)
            
            if matching_subclass_ids:
                # Valida códigos de livro
                valid_codes = {code for code in book_codes if code in book_codes_to_ids}
                
                if valid_codes:
                    # Adiciona relação para TODAS as versões matchadas
                    ...
                    print(f"  ⚠️  Códigos inválidos para {table_subclass_name}: {book_codes}")
            else:
                print(f"  ❓ Sem match: {table_subclass_name}")
    
    # Verifica subclasses ainda faltando e tenta adicionar com PHB padrão
    all_subclass_ids = set(subclasses.keys())
    missing_ids = all_subclass_ids - set(relations.keys())
    
    print(f"\n📋 Adicionando {len(missing_ids)} subclasses restantes com fonte padrão...")
    for sub_id in missing_ids:
        class_name, subclass_name = subclasses[sub_id]
        
        # Lógica para fonte padrão
        if "(UA)" in subclass_name or "UA" in subclass_name:
            default_source = "UA"
        elif "Plane Shift" in subclass_name:
            default_source = "PS:A"  # Ou outro PS conforme apropriado
        else:
            default_source = "PHB"  # Padrão para subclasses oficiais
    ...
    output_file = BASE_DIR / "scripts" / "init_db" / "classes" / "schema_subclass_books_COMPLETE.sql"
    
    print(f"\n📝 Gerando arquivo completo: {output_file}")
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("""-- =========================================================================
-- RELAÇÕES SUBCLASSE-LIVRO COMPLETAS - TODAS AS 219 SUBCLASSES
-- Combina extração de tabelas + casos especiais + fontes padrão
-- =========================================================================
...
""")
        ...
        for subclass_id in sorted(relations.keys()):
            ...
            for book_code in sorted(relations[subclass_id]):
                ...
                # Lógica correta para is_ua_version:
                # Uma relação é UA se a SUBCLASSE contém "(UA)" OU se o livro é UA
                is_ua = 1 if ("(UA)" in subclass_name or book_code == "UA") else 0
        ...
        f.write(f"""
-- ==================== ESTATÍSTICAS FINAIS ====================
-- TODAS as subclasses processadas: {len(relations)} de {len(subclasses)}
-- Relações criadas: {total_relations}
-- Taxa de sucesso: {len(relations)/len(subclasses)*100:.1f}%
-- Método: Tabelas centralizadas + casos especiais + fontes padrão
-- =========================================================================
""")
    ...
    if len(relations) == len(subclasses):
        print("🎉 TODAS AS 219 SUBCLASSES FORAM CAPTURADAS!")
    else:
        print(f"⚠️  Ainda faltam {len(subclasses) - len(relations)} subclasses")
