...
caminho_excel_classe = os.path.join(base_dir, '../../data/excel/classes.xlsx')
caminho_excel_itens = os.path.join(base_dir, '../../data/excel/itens.xlsx')
caminho_excel_backgrounds = os.path.join(base_dir, '../../data/excel/backgrounds.xlsx')
...
def escolhe_classe(lista_atributos):
    ...
    df_classes = pd.read_excel(caminho_excel_classe)
    ...
    def encontrar_classes_por_atributo(lista_atributos, dict_classes_atributos, valor):
        return [nome_classe for nome_classe, index_atributo in dict_classes_atributos.items() if lista_atributos[index_atributo] == valor]


    # Encontra o maior e o segundo maior atributo
    maior_atributo = max(lista_atributos)
    lista_sem_maior = [x for x in lista_atributos if x != maior_atributo]
    maior_atributo2 = max(lista_sem_maior)

    # Tenta encontrar classes com o maior atributo
    lista_classes_sorteio = encontrar_classes_por_atributo(lista_atributos, dict_classes_atributos, maior_atributo)

    # Se não encontrar, tenta com o segundo maior atributo
    if not lista_classes_sorteio:
        lista_classes_sorteio = encontrar_classes_por_atributo(lista_atributos, dict_classes_atributos, maior_atributo2)
...
def cria_dict_armas_completo():
    df_itens = pd.read_excel(caminho_excel_itens)
...
def escolhe_background(lista_livros_ativos):
    df_backgrounds = pd.read_excel(caminho_excel_backgrounds)
...
def informacoes_background(nome_background):
    ...
    def verifica_features(celula, lista_features):
        feature = None

        if 'TITLE' in celula and all(substring not in celula for substring in ['Suggested Characteristics', 'Feature', 'Personality Traits', 'Ideals', 'Bonds', 'Flaws']):
            feature = celula.replace('TITLE', '')
    ...
    for celula in df_backgrounds.iloc[:, 0]:
        ...
        if verificador:
            ...
            verifica_features(celula, lista_features_background)

    print(nome_background, lista_informacoes_background, lista_features_background)
    return lista_informacoes_background, lista_features_background
